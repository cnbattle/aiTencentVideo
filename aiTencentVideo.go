...
// GetVideoUrlForVid 根据vid获取下载链接
func GetVideoUrlForVid(vid string) (string, string, error) {
	...
	if url == "" || fn == "" || fvkey == "" {
		return "", "", errors.New("json解析数据失败")
	}
	return fmt.Sprintf("%v%v?vkey=%v", url, fn, fvkey), fn, nil
}
